        ...
        <div class="card card-primary box-solid">
            <div class="card-header bg-primary with-border">
                <h3 class="card-title text-white">
                    ...
                    <span>{!! $documentable->name !!} {{ trans('dashboard/indexes.document') }}</span>
                </h3>
            </div>
            ...
            <div class="card-body documents-container">
                <div class="row">
                    @forelse($documents as $document)
                        <div class="col-md-2">
                            <div class="well">
                                <a class="btn bttn-info bttn-material-flat bttn-xs" href="{{ $document->url }}" target="_blank"
                                   title="{{ trans('dashboard/general.view_document') }}" data-toggle="tooltip">
                                    <i class="fa fa-eye"></i>
                                </a>
                                ...
                                <form id="form-delete-row{{ $document->id }}" method="POST" action="/dashboard/documents/{{ $document->id }}" class="dt-titan" style="display: inline-block;">
                                    ...
                                    <a data-form="form-delete-row{{ $document->id }}" class="btn bttn-danger bttn-xs bttn-material-flat btn-delete-row" data-toggle="tooltip" title="{{ __('dashboard/forms.delete_document') . $document->name }}"
                    ...
                    @empty
                        <div class="col-md-12">
                            <p class="text-muted  alert alert-info m-t-20 m-b-20">
                                {{ trans('dashboard/forms.document_muted') }}
                                {!! $documentable->name !!}.
                            </p>
                        </div>
                    @endforelse
        ...
        <div class="card card-primary box-solid">
            <div class="card-header bg-primary with-border">
                <h3 class="card-title text-white">
                    ...
                    <span>
                        {{ trans('dashboard/forms.document_upload') }}
                    </span>
                </h3>
            </div>
            ...
            <div class="card-body">
                <div class="callout callout-info callout-help">
                    <h4 class="{{ isRTL() ? 'text-right' : 'title'}}">
                        {{ trans('dashboard/forms.how_it_work_title') }}
                    </h4>
                    <br>
                    <p>
                        {!! trans('dashboard/forms.how_it_work_desc') !!}
                    </p>
                </div>

                <form id="formDocumentDropzone" class="dropzone" method="POST" action="/dashboard/documents/upload" enctype="multipart/form-data">
                    ...
                    <div class="dz-default dz-message">
                        <span>
                            {{ trans('dashboard/forms.document_click_here') }}
                        </span>
                    </div>
                </form>
                    ...
                    <div class="dz-preview dz-file-preview">
                        <a class="dropzone-image-click" href="#">
                            ...
                            <div class="dz-error-mark">
                                <svg width="54px" height="54px" viewBox="0 0 54 54" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:sketch="http://www.bohemiancoding.com/sketch/ns">
                                    n <title> {{ trans('dashboard/forms.error') }} </title>n
                                    <defs></defs>
                                    n
                                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" sketch:type="MSPage">
                                        n
        ...
        <div class="modal-content">
            <div class="modal-header alert-success">
                ...
                <h4 class="modal-title">
                    {{ trans('dashboard/forms.document_upload_name') }}
                </h4>
            </div>
                    ...
                    <fieldset style="padding: 0">
                        <section class="form-group">
                            <label for="modal-document-name">
                                {{ trans('dashboard/forms.name_of_the_document') }}
                            </label>
                            <input type="text" class="form-control" id="modal-document-name"
                                   placeholder="{{ trans('dashboard/forms.document_name_placeholder') }}">
                        </section>
                    </fieldset>
                </form>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button id="btn-form-save" type="button" class="btn btn-primary">
                    {{ trans('dashboard/general.submit') }}
                </button>
            </div>
        </div>
    </div>
        ...
        $(function () {
            ...
            documentDropzone.on("success", function (file, response) {
                if (response.data && response.success) {
                    ...
                    notify("{!!  trans('dashboard/general.successfully')  !!}","{!! trans('dashboard/general.successfully-updated-document') !!}", null, null, 5000);
                } else {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.error) {
                            ...
                            notify("{{ trans('dashboard/general.successfully') }}"," {{trans('dashboard/general.successfully-updated-document-name') }}", null, null, 5000);
