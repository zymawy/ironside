            ...
            <div class="card box-primary box-solid">
                <div class="card-header bg-primary with-border">
                    <h3 class="box-title text-white">
                        ...
                        <span>{{ isset($item)? __('dashboard/forms.edit_the') . $item->title . __('dashboard/forms.entry') : __('dashboard/forms.create_user_text_city') }}</span>
                    </h3>
                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group {{ form_error_class('title', $errors) }}">
                                        ...
                                        <input type="text" class="form-control" id="title" name="title" placeholder="{{ __('dashboard/forms.title_placeholder') }}" value="{{ ($errors && $errors->any()? old('title') : (isset($item)? $item->title : '')) }}">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group {{ form_error_class('province_id', $errors) }}">
                                        <label for="province">{{ __('pagination.previous') }}</label>
                                        {!! form_select('province_id', ([0 => __('dashboard/forms.province_placeholder')] + $provinces), ($errors && $errors->any()? old('province_id') : (isset($item)? $item->province_id : '')), ['class' => 'select2 form-control']) !!}
