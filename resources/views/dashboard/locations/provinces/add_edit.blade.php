            ...
            <div class="card box-solid">
                <div class="card-header bg-primary with-border">
                    <h3 class="card-title text-white">
                        ...
                        <span>{{ isset($item)? __('dashboard/forms.edit_the') . $item->title . __('dashboard/forms.entry') : __('dashboard/forms.create_user_text_province') }}</span>
                    </h3>
                </div>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group {{ form_error_class('title', $errors) }}">
                                        ...
                                        <input type="text" class="form-control" id="title" name="title"
                                               placeholder="{{ __('dashboard/forms.title_placeholder') }}" value="{{ ($errors && $errors->any()? old('title') : (isset($item)? $item->title : '')) }}">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group {{ form_error_class('country_id', $errors) }}">
                                        <label for="country">{{ __('dashboard/indexes.country') }}</label>
                                        {!! form_select('country_id', ([0 => __('dashboard/indexes.country_placeholder')] + $countries), ($errors && $errors->any()? old('country_id') : (isset($item)? $item->country_id : '')), ['class' => 'select2 form-control']) !!}
